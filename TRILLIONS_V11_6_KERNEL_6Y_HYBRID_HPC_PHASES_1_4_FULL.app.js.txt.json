...
async function safeMemoryPressure(){return await Promise.resolve(memoryPressureTracker()).catch(e=>({error:e.message,status:"UNAVAILABLE"}));}
...
async function hpcZetaHealth(){
  ...
  const mem=await memoryPressureTracker().catch?await memoryPressureTracker().catch(e=>({error:e.message})):await memoryPressureTracker();
...
async function hybridMemoryPressure(){
  if(typeof memoryPressureTracker==="function") return await memoryPressureTracker().catch(e=>({status:"UNAVAILABLE",error:e.message}));
  const mu=process.memoryUsage();
  const mem=await si.mem().catch(()=>null);
  const pressure=mem?+(mem.used/mem.total*100).toFixed(2):null;
  return {time:now(),status:pressure==null?"PROCESS_ONLY":"REAL_SYSTEM_MEMORY",pressure_level:pressure==null?"UNKNOWN":pressure>88?"HIGH":pressure>75?"MEDIUM":"NORMAL",process:{rss_MB:+(mu.rss/1048576).toFixed(2),heapUsed_MB:+(mu.heapUsed/1048576).toFixed(2)},system:mem?{used_percent:pressure,total_GB:+(mem.total/1073741824).toFixed(2)}:null};
}
