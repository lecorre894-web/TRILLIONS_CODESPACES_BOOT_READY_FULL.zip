...
async function sendChat(){const msg=document.getElementById('chatmsg').value;const log=document.getElementById('chatlog');log.textContent+='\n\nUSER > '+msg+'\nRUNNING...';try{const r=await fetch('/api/ai-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg})});const j=await r.json();log.textContent+='\nKERNEL > '+(j.answer||JSON.stringify(j,null,2));out.textContent=JSON.stringify(j,null,2)}catch(e){log.textContent+='\nERROR '+e.message}}
async function runTerminal(){const cmd=document.getElementById('terminalcmd').value;const log=document.getElementById('terminallog');log.textContent='$ '+cmd+'\nRUNNING...';try{const r=await fetch('/api/terminal-v11/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cmd,timeout:60000})});const j=await r.json();log.textContent='$ '+cmd+'\nTIME '+(j.duration_ms||0)+' ms / '+(j.duration_sec||0)+' s\nCLASS '+(j.timing_class||'')+'\n\n'+(j.out||'')+(j.err?'\nERR:\n'+j.err:'');out.textContent=JSON.stringify(j,null,2)}catch(e){log.textContent='ERROR '+e.message}}
