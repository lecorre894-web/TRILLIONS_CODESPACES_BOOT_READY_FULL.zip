...
const CHAT_PROVIDER_TIMEOUT_MS=60000;
const HEALTH_TIMEOUT_MS=15000;
const io=new Server(server,{cors:{origin:"*"},maxHttpBufferSize:1e8,pingInterval:25000,pingTimeout:LONG_TIMEOUT_MS,connectTimeout:LONG_TIMEOUT_MS,transports:["websocket","polling"]});
...
async function ai(message){
  ...
  try{
    ...
    const r=await axios.get(url,{timeout:CHAT_PROVIDER_TIMEOUT_MS});
...
async function providerHealth(){const out={time:now(),mode:"REAL_PROVIDER_ONLY",providers:{}};try{const r=await axios.get("https://text.pollinations.ai/ping",{timeout:HEALTH_TIMEOUT_MS});out.providers.pollinations={ok:r.status<500,status:r.status,type:"real_network_ai"}}catch(e){out.providers.pollinations={ok:false,error:e.message,type:"real_network_ai"}};out.providers.openai={ok:!!process.env.OPENAI_API_KEY,type:"real_api_ai",note:process.env.OPENAI_API_KEY?"api_key_present_not_called_by_default":"OPENAI_API_KEY_missing"};out.providers.ollama={ok:false,type:"real_local_ai"};if(process.env.OLLAMA_URL){try{const r=await axios.get(String(process.env.OLLAMA_URL).replace(/\/$/,"")+"/api/tags",{timeout:HEALTH_TIMEOUT_MS});out.providers.ollama={ok:true,status:r.status,type:"real_local_ai",models:r.data&&r.data.models?r.data.models.length:null}}catch(e){out.providers.ollama={ok:false,error:e.message,type:"real_local_ai"}}}else out.providers.ollama.note="OLLAMA_URL_missing";out.any=Object.values(out.providers).some(x=>x.ok);return out}
...
<div class="card wide" id="terminalbox" style="display:none"><h3>TERMINAL SAFE V11</h3><div class="mini">Commande gardée, timing ms/sec, commandes destructives bloquées.</div><input id="terminalcmd" value="node -v && npm -v"><button class="btn" onclick="runTerminal()">RUN TERMINAL V11</button><div id="terminallog" class="out" style="max-height:300px">TERMINAL READY</div></div>
...
async function fetchLong(url,options={},timeoutMs=120000){
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    const r=await fetch(url,{...options,cache:'no-store',signal:controller.signal});
    const text=await r.text();
    try{return JSON.parse(text)}catch(e){return {ok:false,raw:text,status:r.status}}
  }catch(e){return {ok:false,error:e.name==='AbortError'?'TIMEOUT_CLIENT_AFTER_'+timeoutMs+'ms':e.message}}
  finally{clearTimeout(timer)}
}
async function load(u){out.textContent='LOADING LONG '+u;const j=await fetchLong(u,{},120000);out.textContent=JSON.stringify(j,null,2)}
async function askAI(){load('/api/ai?m='+encodeURIComponent(document.getElementById('msg').value))}
async function cmd(c){out.textContent='$ '+c+'\\nRUNNING LONG...';const j=await fetchLong('/api/shell',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cmd:c,timeout:120000})},120000);out.textContent='$ '+c+'\\n\\n'+(j.out||JSON.stringify(j,null,2))+(j.err?'\\n\\nERR:\\n'+j.err:'')}
...
function showChat(){const el=document.getElementById('chatbox');el.style.display=el.style.display==='none'?'block':'none'}
function showTerminal(){const el=document.getElementById('terminalbox');el.style.display=el.style.display==='none'?'block':'none'}
async function sendChat(){const msg=document.getElementById('chatmsg').value;const log=document.getElementById('chatlog');log.textContent+='\n\nUSER > '+msg+'\nRUNNING LONG...';const j=await fetchLong('/api/ai-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg})},120000);log.textContent+='\nKERNEL > '+(j.answer||JSON.stringify(j,null,2));out.textContent=JSON.stringify(j,null,2)}
async function runTerminal(){const cmd=document.getElementById('terminalcmd').value;const log=document.getElementById('terminallog');log.textContent='$ '+cmd+'\nRUNNING LONG...';const j=await fetchLong('/api/terminal-v11/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cmd,timeout:120000})},120000);log.textContent='$ '+cmd+'\nTIME '+(j.duration_ms??0)+' ms / '+(j.duration_sec??0)+' s\nCLASS '+(j.timing_class||'')+'\nTIMEOUT '+(j.timeout_ms??'')+' ms\n\n'+(j.out||'')+(j.err?'\nERR:\n'+j.err:'');out.textContent=JSON.stringify(j,null,2)}
...
app.post("/api/shell",async(req,res)=>{const cmd=String(req.body.cmd||"").trim();const timeout=Math.min(Number(req.body.timeout||LONG_TIMEOUT_MS),LONG_TIMEOUT_MS);if(!cmd)return res.json({ok:false,err:"empty command"});if(blockedCmd(cmd))return res.json({ok:false,err:"blocked dangerous command"});res.json(await sh(cmd,timeout));});
...
app.post("/api/terminal-v11/run",async(req,res)=>res.json(await timedShell(String(req.body&&req.body.cmd||""),Math.min(Number(req.body&&req.body.timeout||LONG_TIMEOUT_MS),LONG_TIMEOUT_MS))));
